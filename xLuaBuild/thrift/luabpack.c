...
#include <inttypes.h>
#include <stdio.h>
//ZHOU He : On windows, use winsock.
#ifdef WIN32
#   include <winsock.h>
#else
...
static int l_bpack(lua_State *L) {
  ...
  switch (code[0]) {
    case 'c': {
      int8_t data = luaL_checkinteger(L, 2);
      luaL_addlstring(&buf, (void*)&data, sizeof(data));
      break;
    }
    case 's': {
      int16_t data = luaL_checkinteger(L, 2);
    ...
    case 'l': {
      int64_t data = luaL_checkinteger(L, 2);
...
static int l_fromVarint64(lua_State *L) {
  ...
  uint64_t n = (uint64_t)luaL_checkinteger(L, 3);
...
int LUALIB_API luaopen_libluabpack(lua_State *L) {
//ZHOU He : Must use setfuncs for lua 5.2+
#if LUA_VERSION_NUM >= 502 // LUA 5.2 or above
    luaL_newlib(L, lua_bpack);
    lua_setglobal(L, "libluabpack");
