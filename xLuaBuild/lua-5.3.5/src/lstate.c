...
static void stack_init (lua_State *L1, lua_State *L) {
  ...
  L1->stack = luaM_newvector(L, BASIC_STACK_SIZE, TValue); //栈上最多还能存40个值？？好像不是这个意思 todo
...
static void f_luaopen (lua_State *L, void *ud) {
  global_State *g = G(L);
  UNUSED(ud);
  stack_init(L, L);  /* init stack 初始化栈 */
...
LUA_API lua_State *lua_newstate (lua_Alloc f, void *ud) {
  ...
  /* 初始化一个线程的栈结构数据 把global_State 挂在lua_State上*/
  preinit_thread(L, g);
  g->frealloc = f;  //内存分配以及释放函数指针
  g->ud = ud;
  g->mainthread = L;
  g->seed = makeseed(L);
  ...
  g->panic = NULL;
  g->version = NULL;
  g->gcstate = GCSpause; //垃圾收集器的初始状态 ？ 默认是暂停，不开启的不进行垃圾收集？？、
  g->gckind = KGC_NORMAL;
  ...
  g->GCdebt = 0;
  g->gcfinnum = 0;
  g->gcpause = LUAI_GCPAUSE; //垃圾回收期的间隔，默认当内存增大2倍时（200/100）执行一次垃圾回收
  g->gcstepmul = LUAI_GCMUL; //收集器单步收集的速度相对于内存分配速度的倍率，设置 200 的倍率等于 2 倍
